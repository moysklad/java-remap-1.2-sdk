...
## 9.3-release (13.05.2026)
* Добавлена сущность подразделения юридического лица `OrganizationBranch` 
* Добавлено поле `organizationBranch` в `RetailStore`, `RetailShift`, `RetailDemand`, `RetailSalesReturn`, `RetailDrawerCashIn`, `RetailDrawerCashOut`, `Prepayment`, `PrepaymentReturn`

## 9.0-release (24.07.2025)
* Изменен тип данных с `int` на `double` для полей `weight` и `volume` в сущности `Bundle` 

## 8.6-release (17.06.2025)
* Добавлено поле `giftCards` в `RetailDemand`

## 8.4-release (24.04.2025)
